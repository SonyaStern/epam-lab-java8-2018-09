...
class Exercise2 {
  ...
  void personHasNotEmptyLastNameAndFirstName() {
    ...
    Predicate<Person> validate = person -> !person.getFirstName().isEmpty() &&
...
//    throw new UnsupportedOperationException();
  ...
  void personHasNotEmptyLastNameAndFirstNameUsingLogicalOperators() {
    Predicate<Person> personHasEmptyFirstName = person -> "".equals(person.getFirstName());
    Predicate<Person> personHasEmptyLastName = person -> "".equals(person.getLastName());

    Predicate<Person> personHasNotEmptyFirstName = negateUsingLogicalOperator(
        personHasEmptyFirstName);
    Predicate<Person> personHasNotEmptyLastName = negateUsingLogicalOperator(
        personHasEmptyLastName);
    ...
    assertThat(personHasNotEmptyLastNameAndFirstName.test(new Person("Алексей", "Доренко", 40)),
        is(true));
    assertThat(personHasNotEmptyLastNameAndFirstName.test(new Person("Николай", "", 30)),
        is(false));
    assertThat(personHasNotEmptyLastNameAndFirstName.test(new Person("", "Мельников", 20)),
        is(false));
  }
...
//    throw new UnsupportedOperationException();
  ...
  void personHasNotEmptyLastNameAndFirstNameUsingStandardMethods() {
    ...
    Predicate<Person> personHasNotEmptyLastNameAndFirstName = personHasNotEmptyFirstName
        .and(personHasNotEmptyLastName);
